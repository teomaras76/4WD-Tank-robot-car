...
extern int16_t gTHR, gRUD, gELE, gAIL;
int16_t motor_thr;
float dt_recip;
...
//  motor_pwm->motor4_pwm = motor_thr - pid->x_s2 + pid->y_s2 - pid->z_s2 + MOTOR_OFF4;
    ...
    if (gELE >= 100){
      if (gAIL >= 0){
        motor_pwm->motor1_pwm = gELE;        /* Fwd pin right motors */
        motor_pwm->motor2_pwm = gELE+gAIL;        /* Fwd pin left motors */
        motor_pwm->motor3_pwm = 0;           /* Rev pin right motors */
        motor_pwm->motor4_pwm = 0;           /* Rev pin left motors */
    }
    if (gAIL < 0){
      motor_pwm->motor1_pwm = gELE-gAIL;       /* Fwd pin right motors */
      motor_pwm->motor2_pwm = gELE;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = 0;           /* Rev pin right motors */
      motor_pwm->motor4_pwm = 0;           /* Rev pin left motors */
    }
  }
  if (gELE <= -100){
    if (gAIL >= 0){
      motor_pwm->motor1_pwm = 0;        /* Fwd pin right motors */
      motor_pwm->motor2_pwm = 0;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = -gELE;           /* Rev pin right motors */
      motor_pwm->motor4_pwm = -gELE+gAIL;           /* Rev pin left motors */
    }
    if (gAIL < 0){
      motor_pwm->motor1_pwm = 0;        /* Fwd pin right motors */
      motor_pwm->motor2_pwm = 0;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = -gELE-gAIL;           /* Rev pin right motors */
      motor_pwm->motor4_pwm = -gELE;           /* Rev pin left motors */
    }
  }
  if (0 <= gELE < 100){
    if (gAIL >= 0){
...
//  motor_pwm->motor4_pwm = motor_thr - pid->x_s2 + pid->y_s2 - pid->z_s2 + MOTOR_OFF4;
        /* Rev Turn right */
  if (gELE >= 0){
    if (gAIL < -MIN_THR){                      /* Fwd Turn right */
      motor_pwm->motor1_pwm = 0;        /* Rev pin right motor-gAILs */
      motor_pwm->motor2_pwm = 0.5*gELE-0.5*gAIL;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = 0.5*gELE;           /* Fwd pin right motors */
      motor_pwm->motor4_pwm = 0;           /* Rev pin left motors */    
    }
    else if (gAIL >= MIN_THR){                     /* Fwd Turn left */
      motor_pwm->motor1_pwm = 0;        /* Rev pin right motors */
      motor_pwm->motor2_pwm = 0.5*gELE;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = 0.5*gELE+0.5*gAIL;           /* Fwd pin right motors */
      motor_pwm->motor4_pwm = 0;           /* Rev pin left motors */    
    }
    else{
      motor_pwm->motor1_pwm = 0;        /* Rev pin right motor-gAILs */
  ...
  if (gELE < 0){
    if (gAIL < MIN_THR){                              /* Rev Turn right */
      motor_pwm->motor1_pwm = -0.5*gELE;        /* Rev pin right motors */
      motor_pwm->motor2_pwm = 0;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = 0;           /* Fwd pin right motors */
      motor_pwm->motor4_pwm = -0.5*gELE-0.5*gAIL;           /* Rev pin left motors */    
    }
    else if (gAIL >= MIN_THR){
      motor_pwm->motor1_pwm = -0.5*gELE+0.5*gAIL;        /* Rev pin right motors */
      motor_pwm->motor2_pwm = 0;        /* Fwd pin left motors */
      motor_pwm->motor3_pwm = 0;           /* Fwd pin right motors */
      motor_pwm->motor4_pwm = -0.5*gELE;           /* Rev pin left motors */    
    }
    else{
      motor_pwm->motor1_pwm = -0.5*gELE;        /* Rev pin right motors */
      motor_pwm->motor2_pwm = 0;        /* Fwd pin left motors */
